...
export class CartView extends Component<ICartViewData> {
	...
	constructor(
		...
		if (!_config.itemFactory) {
			throw new Error(
				'CartView: config.itemFactory not set. Provide correct itemFactory in configuration'
			);
		}
		...
		this._checkoutButton = ensureElement(
			_config.checkoutSelector,
			container
		...
		this.addRenderField('totalCost');
		this.addRenderField('isEmpty');
	}
